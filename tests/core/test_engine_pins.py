"""`scripts/engine-pins.py` feeds `pixi run setup` the commits that reproduce a
frozen verdict table (CI builds main instead -- see the script's docstring). A
wrong commit there installs the wrong engine and quietly makes a row diff
meaningless, so it is tested against the gates' OWN metadata readers rather than
against a second read of the same JSON path: a re-implementation here would
agree with the script while both pointed at the wrong table.
"""
...
def test_the_printed_pins_are_the_commits_the_tables_record():
