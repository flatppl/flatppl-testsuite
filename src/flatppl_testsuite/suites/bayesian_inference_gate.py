"""The bayesian_inference suite: small, self-contained FlatPPL models that
already end in a fixed-point ``lp = logdensityof(posterior, <point>)``
binding, built around the ``bayesupdate`` measure-algebra pipeline.

Like the fragment suite (and unlike the HS3 corpus, a parameterized
likelihood scanned over a theta vector), each posterior needs no
``__score__`` append and no theta: it is scored through the convert-free
det-js path (``flatppl determinize`` -> ``score_flatpdl.cjs``) exactly as
written, and the resulting scalar is compared to a frozen INDEPENDENT oracle
(scipy — see ``corpora/bayesian_inference/gen_expected.py``) recorded in
each posterior's ``expected.json``.
"""
...
BAYESIAN_INFERENCE_CORPUS: Path = _TESTSUITE_ROOT / "corpora" / "bayesian_inference"
BAYESIAN_INFERENCE_MANIFEST: Path = BAYESIAN_INFERENCE_CORPUS / "manifest.json"
...
class PosteriorGateSuite(Suite):
    ...
    def run(self, selected: set[str] | None = None,
        ...
        manifest = json.loads(BAYESIAN_INFERENCE_MANIFEST.read_text())
        results: list[CheckResult] = []

        for post in manifest["posteriors"]:
            test_id = post["test_id"]
            if selected is not None and test_id not in selected:
                continue

            post_dir = BAYESIAN_INFERENCE_CORPUS / post["path"]
            expected_doc = json.loads((post_dir / "expected.json").read_text())
            model_path = post_dir / expected_doc["model"]
